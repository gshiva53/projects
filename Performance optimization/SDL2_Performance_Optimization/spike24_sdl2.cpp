...
struct CrashBox {
	...
	int dx, dy; // vel
	int w, h; // size
	BoxState state = CONTACT_NO;
};

// Global variables. (Apparently evil.)
vector<CrashBox> boxes; 
...
int run_test(const char * title, void (*function_ptr)(), fstream& file) {
  ...
  // renderer for the window ...
	...
	//string fileName = title + (string)".csv"; 
	//Initialize Log file
	//ofstream file; 
	//file.open("Log.csv", fstream::out | fstream::app);

	if (file.fail())
		cout << "Failed to load file for writing..." << endl; 
	...
	//Print all the methods to the file
	//file << "Method , " << title << endl;
	//file << "Loops , " << "Time(ms) , " << "Loops/second , " << endl;
	file << "Vector" << title << "," << loop_count << "," << (tick_end - tick_start) << "," << (float(loop_count) / (tick_end - tick_start) * 1000.0) << endl;
...
{
	fstream file("Log.csv", fstream::out | fstream::app);

	file << "Title , Loops , Time(ms) , Loops/second , " << endl;

	run_test("Test A1", crash_test_all_A1, file); //TODO: not paying attention to return values. :(
	run_test("Test A2", crash_test_all_A2, file);
	run_test("Test B", crash_test_all_B, file);
	run_test("Test C", crash_test_all_C, file);
	run_test("Test D", crash_test_all_D, file);
